...
class Recommender:
	...
	def sampleTrainBatch(self, batIds):
		...
		for i in range(batch):
			...
			if len(posset) == 0 or len(negset) == 0:
				continue
	...
	def trainEpoch(self):
		...
		for i in range(steps):
			...
			uLocs = self.filter_none(uLocs)
			iLocs = self.filter_none(iLocs)

			# Check if we have any valid samples left
			if len(uLocs) == 0 or len(iLocs) == 0:
				continue

			target = [self.optimizer, self.preLoss, self.regLoss, self.loss]
			res = self.sess.run(target, feed_dict={self.uids: uLocs, self.iids: iLocs, self.keepRate: args.keepRate},
